...
public class Controller implements Initializable {
        ...
        importButton.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent mouseEvent) {
                ...
                fileChooser.getExtensionFilters().addAll(
                        new FileChooser.ExtensionFilter("Trajectory Files", "*.trj"));
                File selectedFile = fileChooser.showOpenDialog(button.getScene().getWindow());
                if (selectedFile != null) {
                    try {
                        TrajectoryBuilderWrapper tbw = objectMapper.readValue(selectedFile, TrajectoryBuilderWrapper.class);
                        currentlySelectedIndex = -1;
                        newPose = false;
                        constraints = tbw.driveConstraintsWrapper.toDriveConstraints();
                        poses = new ArrayList<>();
                        for (Pose2dWrapper pose : tbw.pose2dWrapper) {
                            poses.add(pose.toPose2d());
                        }
                        pathName.setText(tbw.name);
                        nodes = new ArrayList<>();
                        while(fieldPane.getChildren().size() > 1) {
                            fieldPane.getChildren().remove(1);
                        }
                        for(Pose2d p : poses) {
                            Circle location = new Circle(4);
                            location.setStroke(Color.BLUE);
                            location.setFill(Color.BLUE);
                            Point2D point = fieldToCanvasSpace(new Pose2d(p.getX(), p.getY(), 0));
                            location.relocate(point.getX(), point.getY());
                            location.setRotate(p.getHeading());
    ...
    private void writeFile(String fileName, String contents) throws IOException{
        File f = new File("C:/Users/Joshua/Documents/path_creation_old/trajectories/" + fileName);
        if (!f.exists()) {
            f.createNewFile();
        }
